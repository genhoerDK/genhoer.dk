...
export default function MapControls({ activeProject, setActiveProject }) {
    return (
        <div className={`absolute bottom-14 right-2 flex gap-2 text-zinc-800 transition-transform duration-500 delay-200 md:hidden ${activeProject ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"}`}>
            <button onClick={() => {setActiveProject(null)}} className="p-2 rounded-full bg-zinc-50 cursor-pointer">
