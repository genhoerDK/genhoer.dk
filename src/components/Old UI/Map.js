...
export default function Map({ projects, width, height }) {
    ...
    return (
        <>
            ...
            {/* Project Cover Image */}
            <div
                className={`absolute inset-0 bg-cover bg-center -z-10 transition-all duration-700 ${isProjectActive ? 'opacity-25' : 'opacity-0'}`}
                style={{ backgroundImage: isProjectActive ? `url(${projects[currentSlug].coverImage})` : 'none', }}
            />
            {/* Map */}
            <svg ref={svgRef} />
        </>
