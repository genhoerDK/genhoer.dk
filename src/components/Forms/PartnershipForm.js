...
export default function PartnershipForm({ onClose }) {
    const { register, handleSubmit, reset, formState: { errors, isSubmitSuccessful, isSubmitting }, } = useForm({ mode: "onSubmit", });

    const [isSuccess, setIsSuccess] = useState(false);

    const apiKey = process.env.NEXT_PUBLIC_WEB3FORMS_KEY || "5d308ce0-fba7-444e-a2e4-135fd2c514bc";
        ...
        <div className="size-full flex justify-center items-center px-4 py-10">
            {!(isSubmitSuccessful && isSuccess) && (
                <form onSubmit={handleSubmit(onSubmit)} className="w-full sm:w-lg p-4 bg-paper">
                    ...
                    <div className="mb-4">
                        <label className="text-xs uppercase">Institution / Organisation</label>
                        <input
                            id="institution"
                            type="text"
                            placeholder=""
                            autoComplete="false"
                            className="w-full px-3 py-2 border-2 outline-none border-muted focus:border-ink"
